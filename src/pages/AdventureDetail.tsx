...
const AdventureDetail: React.FC = () => {
  ...
  const [adventureStatus, setAdventureStatus] = useState<string>('pending');
  ...
  const loadAdventure = async (): Promise<void> => {
    if (!id) return;
    
    try {
      setLoading(true);
      const response = await adventureApi.getAdventure(id);
      if (response.success || response.data) {
        const adventureData = response.data || response;
        setAdventure(adventureData);
        setAdventureStatus(adventureData.status || 'pending');
      } else {
  ...
  const handleStartAdventure = async (): Promise<void> => {
    ...
    try {
      const response = await adventureApi.startAdventure(id);
      if (response.success) {
  ...
  const handleCompleteAdventure = async (): Promise<void> => {
    if (!id) return;
    
    try {
      const response = await adventureApi.completeAdventure(id, feedback);
      if (response.success) {
  ...
  const handleCancelAdventure = async (): Promise<void> => {
    if (!id) return;
    
    try {
      const response = await adventureApi.cancelAdventure(id);
      if (response.success) {
  ...
  const handleCreateEvent = async (step: AdventureStep): Promise<void> => {
    if (!adventure) return;
  ...
  const getStepIcon = (step: AdventureStep): string => {
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{adventure.steps?.length || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Style:</span>
                  <span className="font-medium capitalize">
                    {adventure.socialOption?.replace('_', ' ') || 'solo'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Level:</span>
                  <span className="font-medium capitalize">{adventure.skillLevel || 'beginner'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Created:</span>
                  <span className="font-medium">
                    {adventure.metadata?.generatedAt ? new Date(adventure.metadata.generatedAt).toLocaleDateString() : 'N/A'}
                  </span>
                </div>
              </div>
            </motion.div>
