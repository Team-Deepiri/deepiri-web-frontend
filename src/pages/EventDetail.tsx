...
const EventDetail: React.FC = () => {
  ...
  const isUserRsvped = (): boolean => {
    if (!event || !user) return false;
    return event.participants?.some(p => p._id === user.id || p.id === user.id || p === user.id) || false;
  };

  const isEventFull = (): boolean => {
    if (!event) return false;
    return event.maxParticipants ? (event.participants?.length || 0) >= event.maxParticipants : false;
  };
              ...
              <div className="space-y-6">
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">{event.location?.address || 'Location TBD'}</p>
                  </div>
                </div>
