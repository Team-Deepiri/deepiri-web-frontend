...
interface FormData {
  interests: string[];
  duration: number;
  socialOption: 'solo' | 'friends' | 'host_event';
...
const AdventureGenerator: React.FC = () => {
  ...
  const loadInitialData = async (): Promise<void> => {
    try {
      ...
      const interestsResponse = await adventureApi.getAdventureRecommendations({ lat: 0, lng: 0 }, 0);
      // Note: This API call might not be correct - adjust based on actual API
      setAvailableInterests(['food', 'music', 'art', 'nature', 'nightlife', 'culture', 'sports', 'shopping', 'entertainment']);
  ...
  const handleGenerate = async (): Promise<void> => {
    ...
    try {
      ...
      if (response.success) {
        ...
        navigate(`/adventure/${response.data?._id || response.data?.id}`);
      } else {
      ...
      <div className="max-w-md mx-auto">
        <div className="mb-6">
          ...
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => handleInputChange('duration', parseInt(e.target.value))}
  ...
  const renderStep3 = (): JSX.Element => (
    ...
    >
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {[
          { value: 'solo' as const, label: 'Solo Adventure', icon: '🚶', desc: 'Explore on your own' },
          { value: 'friends' as const, label: 'With Friends', icon: '👥', desc: 'Invite friends to join' },
          { value: 'host_event' as const, label: 'Host Event', icon: '🎉', desc: 'Create a public event' }
        ].map((option) => (
          <button
            key={option.value}
  ...
  const renderStep4 = (): JSX.Element => (
    ...
    >
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {[
          { value: 'beginner' as const, label: 'Beginner', icon: '🌱', desc: 'Easy and relaxed' },
          { value: 'intermediate' as const, label: 'Intermediate', icon: '🚀', desc: 'Moderate challenge' },
          { value: 'advanced' as const, label: 'Advanced', icon: '⚡', desc: 'Full adventure mode' }
        ].map((option) => (
          <button
            key={option.value}
