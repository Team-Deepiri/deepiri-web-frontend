...
const GamificationDashboard: React.FC = () => {
  ...
  const xpPercentage = profile.xp && profile.xpToNextLevel ? (profile.xp / profile.xpToNextLevel) * 100 : 0;
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-right">
              <div className="text-4xl font-bold text-white mb-2">{(profile.points || 0).toLocaleString()}</div>
          ...
          <div className="mb-2">
            <div className="flex justify-between text-sm text-gray-300 mb-2">
              ...
              <span>{profile.xp || 0} / {profile.xpToNextLevel || 100} XP</span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { label: 'Tasks Completed', value: profile.stats?.tasksCompleted || 0, icon: '✅', color: 'from-cyan-400 to-cyan-500' },
            { label: 'Challenges Completed', value: profile.stats?.challengesCompleted || 0, icon: '🎮', color: 'from-purple-400 to-purple-500' },
            { label: 'Total Time Spent', value: `${Math.round((profile.stats?.totalTimeSpent || 0) / 60)}h`, icon: '⏱️', color: 'from-purple-400 to-cyan-400' },
            { label: 'Average Efficiency', value: `${Math.round(profile.stats?.averageEfficiency || 0)}%`, icon: '📈', color: 'from-cyan-400 to-cyan-500' }
          ].map((stat, idx) => (
            <motion.div
              key={idx}
              initial={{ opacity: 0, y: 20 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          >
            ...
            <div className="text-4xl font-bold text-purple-300 mb-2">
              {profile.streaks?.daily?.current || 0} days
            </div>
            <div className="text-gray-400 text-sm">
              Longest: {profile.streaks?.daily?.longest || 0} days
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="text-4xl font-bold text-cyan-300 mb-2">
              {profile.streaks?.weekly?.current || 0} weeks
            </div>
            <div className="text-gray-400 text-sm">
              Longest: {profile.streaks?.weekly?.longest || 0} weeks
            </div>
          </motion.div>
        </div>
