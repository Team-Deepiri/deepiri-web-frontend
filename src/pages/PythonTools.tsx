...
const PythonTools: React.FC = () => {
  const [coords, setCoords] = useState<Coords>({ latitude: '', longitude: '' });
  const [weather, setWeather] = useState<any>(null);
  const [directionsForm, setDirectionsForm] = useState<DirectionsForm>({ origin: '', destination: '', mode: 'driving' });
  const [directions, setDirections] = useState<any>(null);
  const [adventureForm, setAdventureForm] = useState<AdventureForm>({ latitude: '', longitude: '', radius: 5000, interests: '' });
  const [adventureData, setAdventureData] = useState<any>(null);
  const [loading, setLoading] = useState<LoadingState>({ weather: false, directions: false, adventure: false });
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="space-y-3">
              <input value={coords.latitude} onChange={(e: ChangeEvent<HTMLInputElement>) => setCoords((p) => ({ ...p, latitude: e.target.value }))} placeholder="Latitude" className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
              <input value={coords.longitude} onChange={(e: ChangeEvent<HTMLInputElement>) => setCoords((p) => ({ ...p, longitude: e.target.value }))} placeholder="Longitude" className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="space-y-3">
              <input value={directionsForm.origin} onChange={(e: ChangeEvent<HTMLInputElement>) => setDirectionsForm((p) => ({ ...p, origin: e.target.value }))} placeholder="Origin (address or 'lat,lng')" className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
              <input value={directionsForm.destination} onChange={(e: ChangeEvent<HTMLInputElement>) => setDirectionsForm((p) => ({ ...p, destination: e.target.value }))} placeholder="Destination (address or 'lat,lng')" className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
              <select value={directionsForm.mode} onChange={(e: ChangeEvent<HTMLSelectElement>) => setDirectionsForm((p) => ({ ...p, mode: e.target.value }))} className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="space-y-3">
              <input value={adventureForm.latitude} onChange={(e: ChangeEvent<HTMLInputElement>) => setAdventureForm((p) => ({ ...p, latitude: e.target.value }))} placeholder="Latitude" className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
              <input value={adventureForm.longitude} onChange={(e: ChangeEvent<HTMLInputElement>) => setAdventureForm((p) => ({ ...p, longitude: e.target.value }))} placeholder="Longitude" className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
              <input type="number" value={adventureForm.radius} onChange={(e: ChangeEvent<HTMLInputElement>) => setAdventureForm((p) => ({ ...p, radius: Number(e.target.value) }))} placeholder="Radius (meters)" className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
              <input value={adventureForm.interests} onChange={(e: ChangeEvent<HTMLInputElement>) => setAdventureForm((p) => ({ ...p, interests: e.target.value }))} placeholder="Interests (comma-separated)" className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
