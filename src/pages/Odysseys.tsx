...
const Odysseys: React.FC = () => {
  ...
  const handleCompleteMilestone = async (odysseyId: string, milestoneId: string) => {
    try {
      setActionLoading(`complete-${milestoneId}`);
  ...
  const handleAddObjective = async (odysseyId: string, objectiveId: string) => {
    try {
      setActionLoading(`obj-${objectiveId}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <AccessibleModal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="Create Odyssey">
        ...
        {/* Add Milestone Modal */}
        <AccessibleModal
          isOpen={showMilestoneModal && !!selectedOdyssey}
          onClose={() => { setShowMilestoneModal(false); setSelectedOdyssey(null); }}
          title="Add Milestone"
        >
          <form onSubmit={handleAddMilestone}>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 font-medium"
              >
                {actionLoading === 'milestone' ? 'Adding...' : 'Add Milestone'}
              </button>
              <button
                type="button"
                onClick={() => { setShowMilestoneModal(false); setSelectedOdyssey(null); }}
                className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium"
              >
                Cancel
              </button>
