    ...
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { label: 'Tasks Completed', value: stats.totalTasksCompleted || 0, icon: '✅', color: 'from-cyan-400 to-cyan-500' },
            { label: 'Challenges Completed', value: stats.totalChallengesCompleted || 0, icon: '🎮', color: 'from-purple-400 to-purple-500' },
            { label: 'Time Spent', value: `${Math.round((stats.totalTimeSpent || 0) / 60)}h`, icon: '⏱️', color: 'from-purple-400 to-cyan-400' },
            { label: 'Points Earned', value: (stats.totalPointsEarned || 0).toLocaleString(), icon: '⭐', color: 'from-cyan-400 to-cyan-500' }
          ].map((stat, idx) => (
            <motion.div
              key={idx}
              ...
              {Object.entries(stats.tasksByType || {}).map(([type, count], idx) => (
                <div key={idx}>
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${((count as number) / (stats.totalTasksCompleted || 1)) * 100}%` }}
                      transition={{ delay: idx * 0.1 }}
              ...
              {Object.entries(stats.challengesByType || {}).map(([type, count], idx) => (
                <div key={idx}>
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${((count as number) / (stats.totalChallengesCompleted || 1)) * 100}%` }}
                      transition={{ delay: idx * 0.1 }}
            ...
            <div className="space-y-4">
              {analytics[0].insights.map((insight, idx) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-400 capitalize">{insight.type?.replace('_', ' ') || 'insight'}</div>
                </motion.div>
              ))}
            </div>
          </motion.div>
