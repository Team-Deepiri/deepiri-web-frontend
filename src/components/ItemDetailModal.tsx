...
interface ItemDetailModalProps {
  ...
  onAddMemory: (itemId: string, memory: Memory) => Promise<void>;
...
const ItemDetailModal: React.FC<ItemDetailModalProps> = ({ 
  ...
  const handleAddMemory = async (): Promise<void> => {
    if (!newMemory.title.trim() || !item._id) return;
      ...
      <div className="bg-gray-900/95 backdrop-blur-md rounded-2xl border border-white/20 w-full max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">{item.name || 'Unnamed Item'}</h2>
        ...
        <div className="flex flex-col lg:flex-row h-[calc(90vh-120px)]">
          ...
          <div className="lg:w-1/2 flex flex-col">
            ...
            <div className="flex border-b border-white/10">
              {[
                { id: 'details' as const, label: 'Details', icon: EyeIcon },
                { id: 'memories' as const, label: 'Memories', icon: HeartIcon, count: memories.length }
              ].map(tab => (
                <button
                  key={tab.id}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {activeTab === 'details' && (
                <div className="space-y-6">
                  ...
                  {/* Value */}
                  {(item.value?.points && item.value.points > 0) || (item.value?.coins && item.value.coins > 0) || (item.value?.monetaryValue && item.value.monetaryValue > 0) ? (
                    <div>
                      ...
                      <div className="flex gap-4">
                        ...
                        {item.value.coins && item.value.coins > 0 && (
                          <div className="text-center">
                            <div className="text-2xl font-bold text-blue-400">{item.value.coins}</div>
                            <div className="text-sm text-gray-400">Coins</div>
                          </div>
                        )}
                        {item.value.monetaryValue && item.value.monetaryValue > 0 && (
                          <div className="text-center">
                  ...
                  <div className="bg-white/5 rounded-xl p-4 space-y-4">
                    ...
                    <div>
                      <input
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setNewMemory(prev => ({ ...prev, title: e.target.value }))}
                    ...
                    <div>
                      <textarea
                        ...
                        onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setNewMemory(prev => ({ ...prev, description: e.target.value }))}
                        rows={3}
                    ...
                    <div className="flex gap-3">
                      <select
                        ...
                        onChange={(e: ChangeEvent<HTMLSelectElement>) => setNewMemory(prev => ({ ...prev, emotion: e.target.value }))}
                        ...
                        return (
                          <div key={index} className="bg-white/5 rounded-xl p-4 space-y-2">
                            <div className="flex items-center justify-between">
                              <h4 className="font-semibold text-white">{memory.title || 'Untitled Memory'}</h4>
